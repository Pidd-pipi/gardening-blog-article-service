...
func (h *UserHandler) Register(c *gin.Context) {
	...
	user, token, err := h.svc.Register(c.Request.Context(), req.Username, req.Email, req.Password, req.Nickname)
	if err != nil {
		c.Error(err)
		return
	}
...
func (h *UserHandler) UpdateProfile(c *gin.Context) {
	...
	user, err := h.svc.UpdateProfile(c.Request.Context(), userID(c), map[string]string{
		"nickname": req.Nickname, "avatar": req.Avatar, "bio": req.Bio, "social_links": req.SocialLinks,
	})
	if err != nil {
		c.Error(err)
		return
