...
// Register 注册（默认游客）。
func (s *UserService) Register(ctx context.Context, username, email, password, nickname string) (*model.User, string, error) {
	s.log.InfoContext(ctx, constants.LOG_USER_REGISTER_START, "email", email)
	...
	if count > 0 {
		return nil, "", util.ConflictError(constants.MsgDuplicateEmail, errors.New("duplicate email"))
	}
	hash, err := bcrypt.GenerateFromPassword([]byte(password), bcrypt.DefaultCost)
	if err != nil {
		return nil, "", err
	}
	user := &model.User{Username: username, Email: email, PasswordHash: string(hash), Nickname: nickname, Role: constants.RoleVisitor}
	if err := s.repo.Create(user); err != nil {
		return nil, "", util.LogError(s.log, constants.LOG_USER_REGISTER_START, fmt.Errorf("create user: %w", err))
	}
	token, err := util.GenerateToken(s.jwtSecret, s.jwtExpire, user.ID, user.Email, user.Role)
	if err != nil {
		return nil, "", err
	}
...
func (s *UserService) Login(ctx context.Context, email, password string) (*model.User, string, error) {
	...
	if err != nil {
		if errors.Is(err, util.ErrNotFound) {
			return nil, "", util.UnauthorizedError(constants.MsgLoginFailed, errors.New("email not found"))
		}
...
// UpdateProfile 更新资料。
func (s *UserService) UpdateProfile(ctx context.Context, id uint, data map[string]string) (*model.User, error) {
	user, err := s.repo.FindByID(id)
	if err != nil {
		return nil, util.NotFoundError(constants.MsgUserNotFound, err)
	}
	if v, ok := data["nickname"]; ok && v != "" {
		user.Nickname = v
	}
